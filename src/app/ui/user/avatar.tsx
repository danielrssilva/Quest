import { FILL_COLOR_MAPS, classNames } from '@/app/utils';
import { BORDER_COLOR_MAPS } from '@/app/utils/colors';
...
export default function Avatar({ avatar, color }: AvatarProps) {
  return (
    <div className="relative box-border flex items-center justify-center">
      <div
        className={classNames(
          'z-10 rounded-full border border-solid border-accent p-1',
          color && BORDER_COLOR_MAPS[color]
        )}
      >
        <Image
          src={avatar}
      ...
      >
        <path
          ...
          className={classNames(
            (color && FILL_COLOR_MAPS[color]) || 'fill-accent-light'
          )}
        />
      </svg>
    </div>
